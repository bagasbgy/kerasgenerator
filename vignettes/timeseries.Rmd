...
# Introduction to Data Generator for Time Series Model {#intro}
...
sunspot_tbl %>%
  ...
  mutate(
    label = case_when(
      date == target ~ "Target",
      date >= feature_start & date <= feature_end ~ "Feature",
      date > feature_end & date < target ~ "Skipped",
      TRUE ~ "Ignored"
    )
  ) %>%
...
Notice that I set the `feature_start` to be `feature_end - months(timesteps) + months(1)`. This is because the `timesteps` value is representing how long is our feature--so without `+ 1`, the timesteps would be 13.

Basically, the `series_generator()` is transforming your data into this format; the same with `forecast_generator()`, but without `lookback` parameter (see [forecasting section](#forecast) for further explanation). Let's take a look on how those function in action in the following sections.
